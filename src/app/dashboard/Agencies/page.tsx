...
const AgencyPage = () => {
    ...
    const { data, error } = useSWR(`http://localhost:3000/admin/showTourAgencies/${status}`, fetcher);
    ...
    const agencies = Array.isArray(data) ? data : data.agencies || [];

    if (!Array.isArray(agencies)) {
        return <h1>No agencies found</h1>;
    }

    const approve = async (id: string) => {
        try {
            const response = await axios.patch(`http://localhost:3000/admin/acceptTourAgency/${id}`);
    ...
    const reject = async (id: string) => {
        try {
            const response = await axios.delete(`http://localhost:3000/admin/removeTourAgency/${id}`);
            if (response.status === 201) {
